<template>
  <div>
    <div class="flex items-center justify-between mb-4">
      <h3 class="text-lg font-bold text-[var(--color-text-primary)]">Faculties</h3>
        ...
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-[var(--color-bg)]/40 border-b border-[var(--color-border)]">
              ...
              <th class="p-3 text-xs font-semibold text-[var(--color-text-secondary)] uppercase">Dean ID</th>
              <th class="p-3 text-xs font-semibold text-[var(--color-text-secondary)] uppercase">Established</th>
          ...
          <tbody class="divide-y divide-[var(--color-border)]/50">
            <tr v-for="item in items" :key="item.id" class="hover:bg-[var(--color-surface-elevated)]/30 transition-colors">
              ...
              <td class="p-3 text-sm text-[var(--color-text-secondary)] max-w-[200px] truncate">{{ item.description || '-' }}</td>
              <td class="p-3 text-sm text-[var(--color-text-primary)] font-mono text-xs">{{ item.dean_id || '-' }}</td>
              <td class="p-3 text-sm text-[var(--color-text-primary)]">{{ item.established_date ? formatDate(item.established_date) : '-' }}</td>
              <td class="p-3">
            ...
            <tr v-if="items.length === 0">
              <td colspan="7" class="p-6 text-center text-[var(--color-text-tertiary)] text-sm">No faculties found.</td>
            </tr>
          </tbody>
        </table>
      </div>
    ...
    <div v-if="showModal" class="fixed inset-0 z-50 flex items-center justify-center bg-[var(--color-bg)]/80">
      <div class="bg-[var(--color-surface)] border border-[var(--color-border)] rounded-2xl p-6 w-full max-w-lg max-h-[90vh] overflow-y-auto shadow-2xl">
        <div class="flex items-center justify-between mb-5">
          <h3 class="text-lg font-bold text-[var(--color-text-primary)]">{{ editingId ? 'Edit Faculty' : 'Create Faculty' }}</h3>
        ...
        <form @submit.prevent="save" class="space-y-4">
          ...
          <div>
            <label class="block text-xs font-semibold text-[var(--color-text-secondary)] uppercase mb-1">Dean ID</label>
            <input v-model="form.dean_id" class="w-full bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] placeholder:text-[var(--color-text-tertiary)] focus:outline-none focus:border-[var(--color-primary)]" />
          </div>
          <div>
            <label class="block text-xs font-semibold text-[var(--color-text-secondary)] uppercase mb-1">Established Date</label>
            <input v-model="form.established_date" type="date" class="w-full bg-[var(--color-bg)] border border-[var(--color-border)] rounded-lg px-3 py-2 text-sm text-[var(--color-text-primary)] focus:outline-none focus:border-[var(--color-primary)]" />
          </div>
          <div class="flex items-center gap-2">
            <input v-model="form.is_active" type="checkbox" id="faculty-active" class="rounded border-[var(--color-border)] bg-[var(--color-bg)] text-[var(--color-text-primary)] focus:ring-[#93C5FD]" />
            <label for="faculty-active" class="text-sm text-[var(--color-text-primary)]">Active</label>
          </div>
...
const form = ref({ name: '', code: '', description: '', dean_id: '', established_date: '', is_active: true })
...
function openCreate() {
  ...
  form.value = { name: '', code: '', description: '', dean_id: '', established_date: '', is_active: true }
  showModal.value = true
}

function openEdit(item) {
  editingId.value = item.id
  form.value = { name: item.name, code: item.code, description: item.description || '', dean_id: item.dean_id || '', established_date: item.established_date ? item.established_date.slice(0, 10) : '', is_active: item.is_active }
  showModal.value = true
}
...
function confirmDelete(item) {
  if (!confirm(`Delete faculty "${item.name}"? This action cannot be undone.`)) return
  deleteItem(item.id)
}
